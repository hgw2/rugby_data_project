...
number_matches <- length(list.files("3_raw_data/team_data/"))
...
matches %>% 
 ...
 scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(title = "Results of Matches Where Teams Have Kicked More Than The Oppositon",
       x = "Result",
       y = "Percentage") + 
  theme_minimal() +
...
team_rates %>%
  ...
  labs(title = paste("Pearson Correlation =", round(cor(team_rates$kick_rate, team_rates$win_rate), 4)),
       x = "Kick Rate",
       y = "Win Rate") +
   scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
...
team_kicks %>%
  ...
  geom_point() +
 geom_smooth(method = lm, se = FALSE) +
  labs(title = paste("Pearson Correlation =", round(cor(team_kicks$avg._no._kicks_per_game, team_kicks$win_rate), 4)),
